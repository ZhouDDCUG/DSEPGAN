...
from models.mynet import FusionNet
...
def test(opt, model, test_dates, IMAGE_SIZE, PATCH_SIZE):
        ...
        if cur_day == 347:
            for ref_date in test_dates:
                if ref_date != cur_date:
                    ...
                    for i in tqdm(range(len(h_index_list))):
                    ...
                    for real_predict in [real_output]:
                        ...
                        for i in range(6):
                            print('[band %d]: RMSE: %.4f SSIM: %.4f UIQI: %.4f CC: %.4f' % (
                                i + 1,
                                np.array(cur_result['rmse'][i]),
                                np.array(cur_result['ssim'][i]),
                                np.array(cur_result['uiqi'][i]),
                                np.array(cur_result['cc'][i]),
                            ))
    ...
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    im_name = os.path.join(save_dir, 'LGC.tif')
    assert final_im.ndim == 2 or final_im.ndim == 3
    with rasterio.open(im_name, mode='w', **metadata) as dst:
        if final_im.ndim == 3:
            for i in range(final_im.shape[0]):
...
def train(opt, train_dates, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    model_CKPT = torch.load('result_lgc/epoch_best.pth')
...
def main():
    ...
    parser.add_argument('--root_dir', default='/data/ZDD/LGC_npy/', help='Datasets root directory')
    parser.add_argument('--save_dir', default='result_lgc', help='Datasets train directory')
